...
//Library ported to work with the STM32F103 HAL Library
...
bool MPU6050::begin(mpu6050_dps_t scale, mpu6050_range_t range, int mpua)
{
    ...
    McuGenericSWI2C_SelectSlave(mpuAddress);
#endif //USE_SW_SPI
...
/*! \fn MPU6050::setLowPowerSampling
 *  \brief Enables/Disables the LowPower Sampling Mode

 *  \param enable true, to enable the lowPowerSampling mode
 *  \param useGyro true, to leave the Gyro activated during lowpower
 *  \param frequency samplingFrequency in lowPowerMode
*/
void MPU6050::setLowPowerSampling(bool enable, bool useGyro, mpu6050_lpfs_t frequency)
{
    uint8_t value;
    value = readRegister8(MPU6050_REG_PWR_MGMT_1);
    value &= 0b10011111;
    value |= (uint8_t)enable<<5;
    writeRegister8(MPU6050_REG_PWR_MGMT_1, value);

    value = readRegister8(MPU6050_REG_PWR_MGMT_2);
    value &= 0b00111000;
    value |= (uint8_t)!useGyro;
    value |= (uint8_t)!useGyro<<1;
    value |= (uint8_t)!useGyro<<2;
    value |= (uint8_t)frequency<<6;
...
Vector MPU6050::readRawAccel(void)
{
	...
	uint8_t sendData = MPU6050_REG_ACCEL_XOUT_H;
	uint8_t recData[6] = {0,0,0,0,0,0};

#if USE_SW_SPI
	uint16_t nofBytesSent;
	McuGenericSWI2C_SendBlock(&sendData, sizeof(sendData), &nofBytesSent);
	McuGenericSWI2C_RecvBlock(recData, 6, &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, &sendData, sizeof(sendData), 50);
	HAL_I2C_Master_Receive(&hi2c2, mpuAddress<<1, recData, 6, 100);
#endif //USE_SW_SPI

	uint8_t xha = recData[0];
	uint8_t xla = recData[1];
	uint8_t yha = recData[2];
	uint8_t yla = recData[3];
	uint8_t zha = recData[4];
	uint8_t zla = recData[5];
...
{
	uint8_t sendData = MPU6050_REG_GYRO_XOUT_H;
	uint8_t recData[6];
...
#if USE_SW_SPI
	uint16_t nofBytesSent;
	McuGenericSWI2C_SendBlock(&reg, sizeof(reg), &nofBytesSent);
	McuGenericSWI2C_RecvBlock(&value, sizeof(value), &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, (uint8_t*)&reg, sizeof(reg), 300);
	HAL_I2C_Master_Receive(&hi2c2, mpuAddress<<1, (uint8_t*)&value, sizeof(value), 300);
#endif //USE_SW_SPI
...
uint8_t MPU6050::readRegister8(uint8_t reg)
{
	...
	McuGenericSWI2C_RecvBlock(&value, sizeof(value), &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, &reg, sizeof(reg), 50);
	HAL_I2C_Master_Receive(&hi2c2, mpuAddress<<1, &value, sizeof(value), 50);
#endif //USE_SW_SPI
...
#if USE_SW_SPI
	uint16_t nofBytesSent;
	McuGenericSWI2C_SendBlock(data, 2, &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, data, 2, 100);
#endif //USE_SW_SPI
}
...
int16_t MPU6050::readRegister16(uint8_t reg)
{
	...
	uint16_t nofBytesSent;
	McuGenericSWI2C_SendBlock(&reg, sizeof(reg), &nofBytesSent);
	McuGenericSWI2C_RecvBlock(recData, 2, &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, &reg, sizeof(reg), 50);
	HAL_I2C_Master_Receive(&hi2c2, mpuAddress<<1, recData,2, 100);
#endif //USE_SW_SPI

    value = recData[0] << 8 | recData[1];
    return value;
}
...
#if USE_SW_SPI
	uint16_t nofBytesSent;
	McuGenericSWI2C_SendBlock(data, 3, &nofBytesSent);
#else // --> USE_SW_SPI == 0
	HAL_I2C_Master_Transmit(&hi2c2, mpuAddress<<1, data,3, 100);
#endif //USE_SW_SPI
}
